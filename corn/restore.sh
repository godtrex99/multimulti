...
NameUser=$(curl -sS https://raw.githubusercontent.com/godtrex99/permission/main/ipmini | grep $MYIP | awk '{print $2}')
cekdata=$(curl -sS https://raw.githubusercontent.com/backupuserssn/main/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
